...
const DEV_VEND_COLS = 'id,id_vendedor,nome_vendedor,valor_total,data_devolucao'
...
export function useDevolucaoItens(start: string, end: string, enabled: boolean) {
  return useQuery<DevItemRaw[]>(async () => {
    ...
    const data = await buscarTudo<any>((de, ate) => supabase
      .from('vw_ecom_devolucao_externa')
      .select('id,id_vendedor,referencia,nome_produto,grupo,subgrupo,data_devolucao,qtd,valor_total')
      .gte('data_devolucao', start)
      .lte('data_devolucao', end)
      .order('id', { ascending: true })
      .range(de, ate))
