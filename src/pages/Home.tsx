...
import { fmtBRL, fmtNum, mesAbrev, shortName } from '../lib/fmt'
import type { Periodo } from '../types'

interface Props { periodo: Periodo }

const MESES_LABEL = (iso: string) => {
  const d = new Date(iso + 'T12:00:00')
  return ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][d.getMonth()] + '/' + String(d.getFullYear()).slice(2)
}
...
export default function Home({ periodo }: Props) {
  ...
  // Grafico 6 meses por vendedor (excluindo ML/SITE)
  const fat6Vend = useMemo(() => {
    ...
    fat6.forEach((r: any) => {
      if (getCanal(r.nome_vendedor||'') !== 'vendedor') return
      const mes = MESES_LABEL(r.data_faturamento)
      const v   = shortName(r.nome_vendedor)
      if (!byMesVend[mes]) byMesVend[mes] = {}
      byMesVend[mes][v] = (byMesVend[mes][v]||0) + Number(r.faturamento_doc)
      totais.set(v, (totais.get(v)||0) + Number(r.faturamento_doc))
    })
    const top5 = [...totais.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([k])=>k)
    const chartData = Object.keys(byMesVend).sort().map(mes => ({ mes, ...byMesVend[mes] }))
    return { chartData, top5 }
  }, [fat6])

  // Tabela 6 meses por departamento
  const fat6Depto = useMemo(() => {
    if (!fat6) return []
    type Row = { mes: string; Vendedores: number; Marketplace: number; Site: number; total: number }
    const map = new Map<string, Row>()
    fat6.forEach((r: any) => {
      const mes = MESES_LABEL(r.data_faturamento)
      const f   = Number(r.faturamento_doc)
      const canal = getCanal(r.nome_vendedor||'')
      if (!map.has(mes)) map.set(mes, { mes, Vendedores: 0, Marketplace: 0, Site: 0, total: 0 })
        ...
        <Col flex={4}>
          <Card>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead><tr>{['#','Vendedor','Fat.'].map((h,i)=><th key={i} style={{textAlign:i<2?'left':'right',padding:'4px 6px',fontSize:11,color:'var(--text-hint)',fontWeight:600,borderBottom:'1px solid var(--border)',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={fat6Vend.chartData} margin={{top:0,right:0,left:0,bottom:0}}>
                  ...
                  {fat6Vend.top5.map((v,i)=>(
                    <Bar key={v} dataKey={v} stackId="a" fill={COLORS[i%COLORS.length]} radius={i===fat6Vend.top5.length-1?[4,4,0,0]:undefined}/>
                  ))}
                </BarChart>
              </ResponsiveContainer>
            )}
