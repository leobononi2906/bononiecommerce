...
export default function Marketplace() {
  ...
  const itensConta=(id:number)=>painel.filter(p=>p.id_conta===id)
  const res=(id:number)=>{const it=itensConta(id);return{total:it.length,ok:it.filter(i=>i.alerta==='OK').length,atencao:it.filter(i=>i.alerta==='ATENCAO').length,critico:it.filter(i=>i.alerta==='CRITICO'||i.alerta==='ZERADO').length}}
  ...
  async function salvarAjuste(it:PainelItem){
    const v=parseFloat(ajusteVal);if(isNaN(v)||!ajustando){setAjustando(null);return}
    if(ajustando.campo==='estoque'){
      const delta=parseInt(ajusteVal);const novo=Math.max(0,it.estoque_atual+delta)
  ...
  const th:React.CSSProperties={textAlign:'left',padding:'10px 12px',background:C.blueDark,color:'#fff',fontSize:11,fontWeight:600,textTransform:'uppercase',letterSpacing:'.4px',whiteSpace:'nowrap',...font}
  ...
  // Tabela por conta
  const TabelaConta=({conta}:{conta:Conta})=>{
    const it=itensConta(conta.id);const r=res(conta.id)
    return(
      <div style={{marginBottom:32}}>
        {/* Cabeçalho da seção */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12,flexWrap:'wrap',gap:8}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{fontSize:16,fontWeight:700,color:C.blueDark}}>{conta.nome}</div>
            <div style={{fontSize:12,color:C.muted}}>Vendedor ERP: <strong>{conta.id_vendedor_erp||'⚠️ não configurado'}</strong> · Reposição: <strong>{conta.dias_reposicao}d</strong></div>
          </div>
          {/* Badges resumo */}
          <div style={{display:'flex',gap:6}}>
            {[{n:r.total,l:'total',bg:'#EEF2FF',fg:C.blueDark},{n:r.ok,l:'ok',bg:'var(--green-bg)',fg:'var(--green)'},{n:r.atencao,l:'atenção',bg:'var(--amber-bg)',fg:'var(--amber)'},{n:r.critico,l:'crítico',bg:'var(--red-bg)',fg:'var(--red)'}].map(({n,l,bg,fg})=>(
              <span key={l} style={{background:bg,color:fg,fontSize:11,fontWeight:700,padding:'3px 10px',borderRadius:20,...font}}>{n} {l}</span>
            ))}
          </div>
        </div>
        {conta.id_vendedor_erp===0&&(
          <div style={{background:C.amberBg,color:C.amber,border:`1px solid ${C.amber}33`,borderRadius:C.radius,padding:'8px 12px',marginBottom:10,fontSize:12,display:'flex',alignItems:'center',gap:6}}>
            <AlertTriangle size={13}/> Vendedor não configurado — vá em Configurações para definir.
          </div>
        )}
        {it.length===0
          ?<div style={{textAlign:'center',color:C.muted,padding:32,background:C.surface,border:`1px solid ${C.border}`,borderRadius:C.radiusLg,fontSize:13,...font}}>
            <Package size={24} color={C.hint} style={{marginBottom:6}}/><br/>Nenhum produto. Clique em "Adicionar Produto" e selecione <strong>{conta.nome}</strong>.
          </div>
          :<div style={{border:`1px solid ${C.border}`,borderRadius:C.radiusLg,overflow:'hidden'}}>
            <table style={{width:'100%',borderCollapse:'collapse',background:C.surface}}>
              ...
              <tbody>
                {it.map(i=>{
                  const al=ALERTA[i.alerta]
                  const ajEst=ajustando?.id===i.id&&ajustando.campo==='estoque'
                  const ajMed=ajustando?.id===i.id&&ajustando.campo==='media'
                  return(
                    <tr key={i.id} style={{background:i.alerta==='CRITICO'||i.alerta==='ZERADO'?`${C.red}08`:i.alerta==='ATENCAO'?`${C.amber}08`:'transparent'}}>
                      ...
                      <td style={tdR}>
                        {ajMed
                          ...
                          :<span onClick={()=>{setAjustando({id:i.id,campo:'media'});setAjusteVal(String(i.media_diaria_manual??''))}} style={{cursor:'pointer',textDecoration:'underline dotted',color:i.media_diaria_30d===0?C.red:C.muted,fontSize:12}} title="Clique para editar média manual">
                            {i.media_diaria_30d===0?'⚠️ definir':i.media_diaria_30d.toFixed(1)}
                            {i.media_diaria_manual!=null&&<span style={{fontSize:10,color:C.hint}}> (man.)</span>}
                          </span>}
                      </td>
                      ...
                      <td style={td}>
                        <div style={{display:'flex',gap:4}}>
                          ...
                          <button onClick={()=>{setAjustando({id:i.id,campo:'media'});setAjusteVal(String(i.media_diaria_manual??''))}} title="Média manual" style={{padding:'4px 8px',borderRadius:6,border:`1px solid ${C.border}`,background:'transparent',fontSize:11,cursor:'pointer',...font}}>Média</button>
                          {confirmDelete===i.id
                            ...
                            :<button onClick={()=>setConfirmDelete(i.id)} title="Excluir" style={{padding:'4px 8px',borderRadius:6,border:`1px solid ${C.red}33`,background:'transparent',color:C.red,cursor:'pointer',display:'flex',alignItems:'center'}}><Trash2 size={12}/></button>}
                        </div>
                      </td>
                    </tr>
                  )
    ...
    <div style={{padding:24,...font}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <div style={{fontSize:12.5,color:C.muted,marginTop:3}}>Estoque Full · baixa automática diária via ERP · alertas de ruptura</div>
        </div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setAbaConfig(v=>!v)} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',borderRadius:C.radius,border:`1px solid ${abaConfig?C.blueMid:C.border}`,background:abaConfig?'#EFF6FF':'transparent',color:abaConfig?C.blueMid:C.txt,fontSize:13,cursor:'pointer',...font}}>
            <Settings size={14}/> Configurações
          </button>
          <button onClick={()=>{setShowNovo(true);setNovoContaId(contas[0]?.id??null)}} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',borderRadius:C.radius,border:'none',background:C.blueMid,color:'#fff',fontWeight:700,fontSize:13,cursor:'pointer',...font}}>
      ...
      {abaConfig&&(
        <div style={{display:'grid',gap:16,gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',marginBottom:28,padding:20,background:'#F8FAFC',border:`1px solid ${C.border}`,borderRadius:C.radiusLg}}>
          <div style={{gridColumn:'1/-1',fontSize:13,fontWeight:700,color:C.blueDark,marginBottom:4}}>⚙️ Configurações das Contas</div>
          {contas.map(c=>(
            <div key={c.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:C.radius,padding:16}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                <div style={{fontSize:14,fontWeight:700,color:C.blueDark}}>{c.nome}</div>
                {editConta===c.id
                  ?<div style={{display:'flex',gap:6}}>
                    <button onClick={()=>salvarConta(c.id)} style={{display:'flex',alignItems:'center',gap:4,padding:'5px 10px',borderRadius:7,border:'none',background:C.blueMid,color:'#fff',fontSize:12,cursor:'pointer',...font}}><Save size={11}/> Salvar</button>
                    <button onClick={()=>setEditConta(null)} style={{padding:'5px 8px',borderRadius:7,border:`1px solid ${C.border}`,background:'transparent',cursor:'pointer'}}><X size={11}/></button>
                  </div>
                  :<button onClick={()=>{setEditConta(c.id);setEditVal({id_vendedor_erp:c.id_vendedor_erp,dias_reposicao:c.dias_reposicao})}} style={{display:'flex',alignItems:'center',gap:4,padding:'5px 10px',borderRadius:7,border:`1px solid ${C.border}`,background:'transparent',fontSize:12,cursor:'pointer',...font}}>
                    <Pencil size={11}/> Editar
                  </button>}
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3,textTransform:'uppercase',letterSpacing:'.4px',fontWeight:600,...font}}>ID Vendedor ERP</div>
                  {editConta===c.id
                    ...
                    :<div style={{fontSize:13,fontWeight:600,color:c.id_vendedor_erp===0?C.red:C.txt}}>{c.id_vendedor_erp===0?'⚠️ Não configurado':c.id_vendedor_erp}</div>}
                </div>
                <div>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3,textTransform:'uppercase',letterSpacing:'.4px',fontWeight:600,...font}}>Reposição (dias)</div>
                  {editConta===c.id
                    ...
                    :<div style={{fontSize:13,fontWeight:600}}>{c.dias_reposicao}d</div>}
                </div>
              </div>
              <div style={{marginTop:10,fontSize:11,color:C.muted}}>
                🟡 Atenção &lt; {Math.round(c.dias_reposicao*1.5)}d · 🔴 Crítico &lt; {c.dias_reposicao}d
              </div>
            </div>
          ))}
        </div>
      ...
      {/* TABELAS LADO A LADO (ou empilhadas em tela pequena) */}
      {contas.map(c=><TabelaConta key={c.id} conta={c}/>)}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div style={{background:'#fff',borderRadius:C.radiusLg,padding:28,width:440,boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'grid',gap:12}}>
              ...
              {([['referencia','SKU / Referência *','ex: 12345'],['nome_produto','Nome do Produto','ex: Geladeira Stonni 30L'],['id_anuncio_ml','ID Anúncio ML (opcional)','ex: MLB1234567890']] as [keyof NovoItem,string,string][]).map(([k,l,ph])=>(
                <div key={k}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                ...
                <div>
                  <label style={{fontSize:11,fontWeight:600,color:C.muted,textTransform:'uppercase',letterSpacing:'.4px',display:'block',marginBottom:5,...font}}>Mínimo</label>
                ...
                <div>
                  ...
                  <input type="number" step="0.1" value={novoItem.media_diaria_manual} onChange={e=>setNovoItem(p=>({...p,media_diaria_manual:e.target.value}))} placeholder="auto" style={inp}/>
                </div>
              </div>
              <div style={{background:'#F8FAFC',borderRadius:8,padding:'10px 14px',fontSize:12,color:C.muted}}>
                💡 <strong>Venda/dia:</strong> deixe em branco — o sistema calcula automaticamente pelos últimos 30 dias do ERP. Preencha só se quiser forçar um valor.
              </div>
            </div>
