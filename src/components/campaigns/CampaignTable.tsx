...
import { ChevronRight, AlertTriangle, TrendingUp, Pause, Eye, EyeOff } from 'lucide-react'
...
export default function CampaignTable({ campaigns, details, summary }: Props) {
  ...
  const visible = hidePaused ? campaigns.filter(c => !c.isPaused) : campaigns
  const filtered = filter === 'TODOS' ? visible : visible.filter(c => c.verdict === filter)
  const maxSpend = Math.max(...campaigns.map(c => c.spend), 1)
  const urgentes = visible.filter(c => c.verdict === 'PAUSAR' && c.spend > 500)
  const pausedCount = campaigns.filter(c => c.isPaused).length
  ...
  function getDetails(camp: string) {
    const norm = normCamp(camp)
    const rows = details.filter(d => normCamp(d.campanha) === norm)
          ...
          {pausedCount > 0 && (
            <button
              ...
              style={{
                ...
                background: hidePaused ? 'var(--amber-bg)' : 'transparent',
                color: hidePaused ? 'var(--amber)' : 'var(--text-muted)',
                ...font
              }}
            >
              {hidePaused ? <EyeOff size={12} /> : <Eye size={12} />}
              {hidePaused ? `${pausedCount} pausadas ocultas` : 'Ocultar pausadas'}
            </button>
          )}
              ...
              return (
                <React.Fragment key={c.campanha}>
                  <tr style={{ background: rowBg, cursor: has ? 'pointer' : 'default', borderBottom: '1px solid var(--border)' }} onClick={() => has && toggle(c.campanha)}>
                    ...
                    <td style={{ ...tdM, fontSize: 12, color: c.pctInvestFat === 0 ? 'var(--text-hint)' : c.pctInvestFat <= 7 ? 'var(--green)' : c.pctInvestFat <= 15 ? 'var(--amber)' : 'var(--red)' }}>{c.pctInvestFat > 0 ? fmtPct(c.pctInvestFat, 1) : '–'}</td>
                    <td style={{ ...tdM, fontSize: 13, color: vc.fg }}>{c.roas > 0 ? `${c.roas.toFixed(1)}x` : '–'}</td>
