    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-mono)', color: labelColor }}>{value.toFixed(1)}%</span>
      ...
      {/* Gauge bar */}
      <div style={{ position: 'relative', height: 18, borderRadius: 20, overflow: 'hidden', background: 'var(--surface-sunken)' }}>
        ...
        <div style={{ position: 'absolute', top: -2, bottom: -2, left: `${pct}%`, width: 3, background: labelColor, borderRadius: 2, boxShadow: `0 0 8px ${labelColor}` }}>
          <div style={{ position: 'absolute', top: -3, left: -5, width: 13, height: 13, borderRadius: '50%', border: '2px solid var(--surface-card)', background: labelColor }} />
        </div>
      </div>
